...
public class Interpreter {
    ...
    private Statement parsePrintStatement(String line) {
        ...
        if (printMatcher.matches()) {
            ...
            //       PrintStatement.
            Expression expressionLineValue = Expression.create(expressionLine);
            return new PrintStatement(expressionLineValue);
        }
    ...
    private Statement parseReturnStatement(String line) {
        ...
        if (returnMatcher.matches()) {
            ...
            Expression returnedMatcher = Expression.create(returnLine);
            return new ReturnStatement(returnedMatcher);
    ...
    private Statement parseWhileStatement(String line, Queue<String> lines) {
        ...
        if (whileMatcher.matches()) {
            ...
            return new WhileStatement(condition , loopStatements);
        }
    ...
    private Statement parseForStatement(String line, Queue<String> lines) {
        ...
        if (forMatcher.matches()) {
           ...
           // ForStatement(Condition condition, List< java.sql.Statement > blockStatement, java.sql.Statement updatedState, java.sql.Statement initializedState)
            Condition condition = Condition.create(conditionLine);
            Statement initializationStatement = parseAssignStatement(initializationLine);
            Statement updatelineStatement = parseAssignStatement(updateLine);

            return new ForStatement(condition,updatelineStatement,initializationStatement, loopStatements);
